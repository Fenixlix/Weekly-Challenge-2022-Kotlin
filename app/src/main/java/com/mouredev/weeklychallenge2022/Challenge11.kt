...
private fun findNonCommon(str1: String, str2: String): String {
    ...
    str1.lowercase(Locale.ROOT).forEach {
        if (!str2.lowercase(Locale.ROOT).contains(it)) {
            out += it
        }
    }
...
private fun printNonCommonWithFilter(str1: String, str2: String) {
    println("out1: ${str1.lowercase(Locale.ROOT).filter { !str2.lowercase(Locale.ROOT).contains(it) }}")
    println("out2: ${str2.lowercase(Locale.ROOT).filter { !str1.lowercase(Locale.ROOT).contains(it) }}")
}